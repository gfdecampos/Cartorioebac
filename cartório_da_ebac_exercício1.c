...
{
	//Início criação de variávies/string
	char arquivo[40];
	char cpf[40];
	char nome[40];
	char sobrenome[40];
	char cargo[40];
	//Final criação de variávies/string
	...
	fprintf(file, ",");//Adiciona uma vírgula para não ficar todos os dados juntos
	fclose(file);
	...
	file = fopen(arquivo, "a"); //Adiciona uma vírgula para não ficar todos os dados juntos
	fprintf(file, ",");
	fclose(file);
